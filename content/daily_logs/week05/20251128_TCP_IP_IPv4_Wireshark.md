...
|:---:|:---|:---|:---|
| **1** | **OSI 7계층 vs TCP/IP 모델** | OSI는 7계층(물리, 데이터링크, 네트워크, 전송, 세션, 표현, 응용)으로 이론적 모델이며, TCP/IP는 4계층(네트워크 접근, 인터넷, 전송, 응용)으로 실제 인터넷 프로토콜 스택입니다. 각 계층은 **캡슐화(Encapsulation)**를 통해 상위 계층 데이터를 하위 계층 헤더로 감쌉니다. | **공격 벡터 분석**의 기초입니다. 각 계층별로 서로 다른 공격이 존재합니다: L2(ARP Spoofing), L3(IP Spoofing, Routing 공격), L4(SYN Flooding, Port Scanning), L7(SQL Injection, XSS). 계층별 이해는 방어 전략 수립의 출발점입니다. |
